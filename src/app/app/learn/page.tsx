...
const Learn = async () => {
  ...
  const featured = trainingItems?.filter((item) => {
  ...
  const training = trainingItems?.filter((item) => {
    return item.category === "training";
  });

  const papers = trainingItems?.filter((item) => {
    return item.category === "papers";
  });
  ...
  return (
    <Page>
      ...
      <div className="flex flex-col w-full gap-4 mx-auto max-w-screen-xl px-4 py-8 sm:px-6 lg:px-8 justify-between">
        <p className="font-bold text-gray-900 sm:text-xl">Featured</p>
          ...
          {featured?.map((item) => {
            return (
              <Link
                ...
                href={item.external_url ?? `/app/learn/${item.slug}`}
        ...
        <p className="font-bold text-gray-900 sm:text-xl mt-4">White Papers</p>
        <div className="flex w-full gap-8">
          {papers?.map((item) => {
            return (
              <Link
